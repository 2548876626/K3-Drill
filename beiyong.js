// app.js for K3-Drill (V12 - with Clear Favorites/Mistakes, Complete)
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const updateUIOnStateChange = () => {
        ...
        const show = !!user;
        showFavoritesBtn.style.display = show ? 'inline-flex' : 'none';
        clearFavoritesBtn.style.display = show ? 'inline-flex' : 'none';
        showMistakesBtn.style.display = show ? 'inline-flex' : 'none';
        clearMistakesBtn.style.display = show ? 'inline-flex' : 'none';
        identityMenu.style.display = show && user.token ? 'block' : 'none';
        guestLoginBtn.style.display = show ? 'none' : 'inline-block';
        if (show && user.isGuest) { const welcome = document.createElement('div'); welcome.className = 'guest-welcome'; welcome.innerHTML = `<span>游客模式</span> <button id="exit-guest-btn">退出</button>`; identityMenu.parentNode.insertBefore(welcome, guestLoginBtn); } 
        else if (!show) { identityMenu.style.display = 'block'; }
        if (!show) resetOrder(); else renderQuestions(false);
    };
    ...
    const renderQuestions = (resetAll = true) => {
        ...
        appState.currentQuestions.forEach((q, index) => {
            ...
            questionDiv.innerHTML = `<div class="question-title"><span>${q.title}</span><button class="favorite-btn ${isFavorited ? 'favorited' : ''}" data-q-num="${q.questionNumber}" title="收藏/取消收藏">⭐</button></div><div class="question-image"><img src="${q.image}" alt="题目图片" loading="lazy"></div><div class="options-list">${optionsHTML}</div><div class="action-buttons"><button class="submit-btn">确定</button><button class="reveal-btn">查看答案</button></div><div class="answer-area"></div>`;
            questionArea.appendChild(questionDiv);
        });
        showQuestion(resetAll ? 0 : appState.currentIndex);
    };

    const checkAnswer = (index) => { const questionData = appState.currentQuestions[index]; const questionEl = document.getElementById(`q-${index}`); const answerArea = questionEl.querySelector('.answer-area'); const selectedValues = Array.from(questionEl.querySelectorAll('input:checked')).map(input => input.value).sort(); const correctValues = questionData.correctAnswer.split('┋').map(ans => ans.charAt(0)).sort(); const isCorrect = selectedValues.length === correctValues.length && selectedValues.every((v, i) => v === correctValues[i]); answerArea.innerHTML = `<strong>${isCorrect ? '回答正确！' : '回答错误！'}</strong><br>正确答案：${questionData.correctAnswer}`; answerArea.className = `answer-area ${isCorrect ? 'correct' : 'incorrect'}`; if (!isCorrect && appState.currentUser) { if (!appState.userData.mistakes.includes(questionData.questionNumber)) { appState.userData.mistakes.push(questionData.questionNumber); debouncedSaveUserData(); showToast("已加入错题本！"); } } };
    const showQuestion = (index) => { if(index >= 0 && index < appState.currentQuestions.length) { appState.currentIndex = index; document.querySelectorAll('.question-item').forEach((item, i) => item.classList.toggle('active', i === index)); counterEl.textContent = `${index + 1} / ${appState.currentQuestions.length}`; prevBtn.disabled = index === 0; nextBtn.disabled = index === appState.currentQuestions.length - 1; }};
    const revealAnswer = (index) => { const questionEl = document.getElementById(`q-${index}`); const answerArea = questionEl.querySelector('.answer-area'); answerArea.innerHTML = `<strong>正确答案：</strong>${appState.currentQuestions[index].correctAnswer}`; answerArea.className = 'answer-area revealed'; };
    const setMode = (mode) => { appState.currentMode = mode; practiceModeBtn.classList.toggle('active', mode === 'practice'); reviewModeBtn.classList.toggle('active', mode === 'review'); footer.style.display = (mode === 'review') ? 'none' : 'flex'; document.querySelectorAll('.question-item').forEach((item, idx) => { item.classList.toggle('active', mode === 'review' || idx === appState.currentIndex); item.querySelector('.action-buttons').style.display = (mode === 'practice') ? 'flex' : 'none'; const answerArea = item.querySelector('.answer-area'); answerArea.className = 'answer-area'; const correctValues = appState.currentQuestions[idx].correctAnswer.split('┋').map(ans => ans.charAt(0)); item.querySelectorAll('.option-item').forEach(label => { const shouldHighlight = mode === 'review' && correctValues.includes(label.dataset.optionChar); label.classList.toggle('correct-highlight', shouldHighlight); if (mode === 'practice') label.classList.remove('selected'); }); }); if (mode === 'practice') showQuestion(appState.currentIndex); };
    ...
    const filterQuestions = (type) => { allControlButtons.forEach(btn => btn.classList.remove('active')); const targetBtn = type === 'favorites' ? showFavoritesBtn : showMistakesBtn; targetBtn.classList.add('active'); practiceModeBtn.classList.add('active'); const idList = appState.userData[type]; if (!idList || idList.length === 0) { alert(`你的${type === 'favorites' ? '收藏夹' : '错题本'}是空的！`); resetOrder(); return; } appState.currentQuestions = appState.originalQuestions.filter(q => idList.includes(q.questionNumber)); renderQuestions(false); setMode('practice'); };
    ...
    function setupEventListeners() {
        ...
        questionArea.addEventListener('click', e => {
            ...
            const index = parseInt(questionItem.id.split('-')[1]);
            if (e.target.classList.contains('favorite-btn')) {
                ...
                if (isFavorited) { if (favIndex === -1) appState.userData.favorites.push(qNum); } 
