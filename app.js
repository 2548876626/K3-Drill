// app.js for K3-Drill (V16 - Final Init & Event Fix)
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const setUser = async (user) => { appState.isDataLoaded = false; if (user) { appState.currentUser = user.token ? user : { sub: user.isGuest ? user.id : getOrCreateGuestId(), isGuest: true, token: null }; await fetchUserData(); } else { appState.currentUser = null; appState.userData = { favorites: [], mistakes: [] }; } appState.isDataLoaded = true; dispatchStateChange(); };
